      ...
      <div className="section-container !py-14 sm:!py-20">
        <AnimatedSection>
          <div className="grid gap-10 lg:grid-cols-[minmax(0,1.15fr)_minmax(18rem,0.85fr)] lg:items-end">
            ...
            <div className="border-t border-[var(--verde-profundo)] pt-6">
              <p className="text-xs font-extrabold uppercase tracking-[0.12em] text-[var(--verde-hoja)]">Tu apoyo acompaña</p>
              <ul className="mt-5 divide-y divide-[var(--border)]">
                {ways.map((way) => (
                  <li key={way} className="py-3 font-semibold text-[var(--verde-profundo)]">{way}</li>
                ))}
              </ul>
              <Link href="/donaciones" className="action-primary mt-7">Cómo colaborar <ArrowRight size={18} /></Link>
            </div>
          </div>
        </AnimatedSection>
      </div>
